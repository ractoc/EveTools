...
public class UserAuthenticationFilter extends AbstractAuthenticationProcessingFilter {
    ...
    public UserAuthenticationFilter(final RequestMatcher requiresAuth) {
        super(requiresAuth);
    }
    ...
    public Authentication attemptAuthentication(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse) {
        return Optional.ofNullable(httpServletRequest.getHeader(AUTHORIZATION))
                ...
                .map(at -> getAuthenticationManager().authenticate(at))
                .orElseThrow(() -> new AuthenticationCredentialsNotFoundException("No eve-state provided in Authorization header"));
    }
